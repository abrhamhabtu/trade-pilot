...
export const BarChartComponent: React.FC<BarChartComponentProps> = ({ data, title }) => {
  ...
  // Show fewer bars for smaller display
      ...
      <div className="relative z-10 h-full flex flex-col">
        ...
        <div className="px-5 pt-4 pb-3">
          <div className="flex items-center space-x-2">
            ...
            <Tooltip content={tooltipContent} position="top">
              <div className="w-5 h-5 rounded-full bg-[#242838] flex items-center justify-center cursor-help hover:bg-emerald-500/20 transition-colors">
        ...
        <div className="flex-1 px-4 pb-2 flex min-h-0">
          ...
          <div className="flex flex-col justify-between text-right pr-3 py-2" style={{ minWidth: '50px' }}>
            {yAxisLabels.map((label, idx) => (
              <span key={idx} className="text-[11px] text-zinc-400 font-medium">
                {formatYLabel(label)}
              </span>
            ))}
          </div>
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
              {yAxisLabels.map((_, idx) => (
                <div key={idx} className="border-t border-white/5 w-full" />
              ))}
            </div>
            ...
            {hasNegative && (
              <div
                className="absolute left-0 right-0 border-t-2 border-white/10"
                style={{ top: '50%' }}
              />
            )}
              ...
              {displayData.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isHovered && (
                      <div
                        className="absolute z-50 px-3 py-2 rounded-lg shadow-xl pointer-events-none whitespace-nowrap bg-[#242838] border"
                        style={{
                      ...
                      >
                        ...
                        <div className={`text-lg font-bold ${isPositive ? 'text-emerald-500' : 'text-rose-500'}`}>
                    ...
                    >
                      ...
                      <div
                        className={`transition-all duration-300 cursor-pointer ${isHovered ? 'brightness-110' : 'opacity-90 hover:opacity-100'
                          }`}
                        style={{
        ...
        <div className="px-4 pb-4 flex justify-around" style={{ marginLeft: '50px' }}>
          {displayData.map((item, idx) => (
            <span
              key={idx}
              className={`text-[11px] font-medium transition-colors ${hoveredIndex === idx ? 'text-zinc-100' : 'text-zinc-400'
                }`}
